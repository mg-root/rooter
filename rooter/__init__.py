...
class Rooter:
    ...
    def addColor(self, name, hex=False, rgb=False):
        if hex:
            ...
            self.tags.append({'start': f'<{name}>', 'end': f'</{name}>', 'value': color})
            print(f"{color[0]};{color[1]};{color[2]}m")
        elif rgb:
            r,g,b = rgb
            self.colors[name] = f'\033[38;2;{r};{g};{b}m'
            self.tags.append({'start': f'<{name}>', 'end': f'</{name}>', 'value': f'\033[38;2;{r};{g};{b}m'})
...
def print(message, styles=False):
    if isinstance(message, str):
        ...
        message = rooter.colors['green'] + str(message) + rooter.reset if message else rooter.colors['red'] + str(message) + rooter.reset
    elif message == None:
        message = rooter.colors['purple'] + str(message) + rooter.reset
